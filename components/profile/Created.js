...
export default function Created({ loading, data, isMyProfile, setSaleModal }) {
                    ...
                    {loading ?
                        ...
                        : data?.length === 0 ?
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-6 gap-6" >
                                {data?.map((history) => (
                                    <NFT isDisplayCard={true} type="created" asset={history} transaction={history} onClick={() => {
                                        if (isMyProfile) {
                                            setSaleModal({
                                                nftData: history,
                                                quantity: parseInt(history.tokenQty) - parseInt(history.soldQty)
                                            })
                                        } else {
