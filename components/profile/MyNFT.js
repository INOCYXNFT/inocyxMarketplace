...
export default function MyNFT({ loading, data, setSaleModal, isMyProfile }) {
      ...
      <div className="grid grid-cols-12 w-full mt-6 gap-6" >
            ...
            : data?.length === 0 ?
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6" >
                {data?.map((history, index) => (
                  <NFT staked={true} key={index} isDisplayCard={true} type="collected" asset={history?.nftData} transaction={history}
                    onClick={() => {
                      if (isMyProfile) {
                        setSaleModal(history)
                      } else {
                        Router.push(`/asset/${history?.nftData?._id}`)
                      }
                    }} />
                ))}
        ...
        {/* <div className="col-span-3 border-[1px] border-white/10 rounded-xl" >
          <Accordion defaultExpanded className="bg-transparent shadow-none border-b-[1px] border-white/5 rounded-none p-3" style={{ backgroundImage: "none" }} >
            ...
            <AccordionDetails className="flex flex-row gap-4 items-center flex-wrap" >
          ...
          <Accordion className="bg-transparent shadow-none border-b-[1px] border-white/5 rounded-none p-3" style={{ backgroundImage: "none" }} >
            ...
            <AccordionDetails className="flex flex-row gap-2 items-center flex-wrap" >
